...
const processZip = (stream: ReadableStream, binName: string, path: string): WritableStream =>
  stream.pipe(extractZip(new RegExp(binName), { forceStream: true }))
    .pipe(createWriteStream(join(path, binName)));

const processTarGz = (stream: ReadableStream, binName: string, path: string): WritableStream =>
  stream.pipe(createGunzip())
    .pipe(extractTar({ cwd: path }, [binName]));

const processBin = (stream: ReadableStream, binName: string, path: string): WritableStream =>
  stream.pipe(createWriteStream(join(path, binName)));

export const unpackArchive = (stream: ReadableStream, { url, binName }: PackageInfo, path: string): Promise<void> => {
